...
def _get_world_matrix_from_translation_vec(translation_vec, rotation):
    cr = rotation.copy()
    cr = cr.transpose()
    camera_center = - np.dot(cr, translation_vec)
    camera_rotation = np.c_[cr, camera_center]
    camera_rotation = np.r_[camera_rotation, np.array([[0, 0, 0, 1]])]

    return camera_rotation

def get_cam_parameters(camera):
    fov = camera.get_field_of_view()
    shift = compute_principal_point_shift(
        camera, relativ_to_largest_extend=True
    )
    focal_length = camera.get_focal_length()
    res = (camera.width, camera.height)
    dist = camera._radial_distortion
