...
def colmap2texrecon(colmap_path, images_path):
    ...
    colmap_files = glob.glob(f"{colmap_path}*.bin")
    ...
    colmap_images = read_images_text(f"{colmap_path}images{suffix}")

    print(f"Results to be stored in dir : {images_path}")
    ...
    for _, colmap_image in tqdm.tqdm(colmap_images.items()):
        ...
        with open(out_file_path, 'w') as out_file:
            out_line = (
                f"{str(tcw[0])} {str(tcw[1])} {str(tcw[2])} {str(R[0, 0])} {str(R[0, 1])} {str(R[0, 2])} {str(R[1, 0])} {str(R[1, 1])} {str(R[1, 2])} {str(R[2, 0])} {str(R[2, 1])} {str(R[2, 2])}"
                + "\n"
            )
